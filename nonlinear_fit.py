...
def map_NL_to_A(NL):
    """
    Map the nonlinearity parameter NL to its corresponding best fit parameter A.

    Args:
        NL: (float) Nonlinearity label (can be positive or negative)

    Returns:
        (float) Best fit parameter A (with the same sign as NL)
    """
    if NL == 0:
        return np.inf
    # This normalized paramA table corresponds to nonlinearity label from 0.01 to 9, with step=0.01 (taken from formula.cpp)
    A_list = np.array([	126.268958,	63.134314,	42.089359,	31.566827,	25.253264,	21.044185,	
						...
						0.025738,	0.025468,	0.025199,	0.024930,	0.024663,	0.024396,	
						0.024129,	0.023864,	0.023599,	0.023336,	0.023073,	0.022810])
    sign = 1 if NL >= 0 else -1
    index = int(abs(NL) * 100) - 1
    index = min(index, len(A_list) - 1)
    index = max(index, 0)
    return sign * A_list[index]
